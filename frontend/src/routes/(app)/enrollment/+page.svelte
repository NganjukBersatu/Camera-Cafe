<script lang="ts">
	...
	type FaceValidation = 'valid' | 'no_face' | 'multiple_faces' | 'blur' | null;

	let step = $state<Step>('form');
	let submitting = $state(false);
	let error = $state<string | null>(null);
	let faceValidation = $state<FaceValidation>(null);
	...
	async function submit(): Promise<void> {
		...
		try {
			const result = await api.enrollment.submit(formData);
			enrolledCustomerId = result.customer_id;
			step = 'done';
		} catch (e) {
...
<div class="mx-auto max-w-xl">
	...
	{#if step === 'done' && enrolledCustomerId}
		<div class="rounded-xl border border-success-700 bg-success-900/20 p-8 text-center">
			...
			<p class="mt-1 text-sm text-surface-400">Pelanggan telah terdaftar dan siap dikenali kamera.</p>
			<div class="mt-6 flex gap-3 justify-center">
				...
				<button
					onclick={() => { step = 'form'; name = ''; contact = ''; preferences = ''; notes = ''; selectedFile = null; previewUrl = null; enrolledCustomerId = null; }}
	...
	{:else}
		...
		>
			...
			<fieldset class="flex flex-col gap-3">
				...
				{#if faceValidation === 'no_face'}
					<p class="text-sm text-error-400">⚠ Tidak ada wajah terdeteksi. Gunakan foto yang lebih jelas.</p>
				{:else if faceValidation === 'multiple_faces'}
					<p class="text-sm text-error-400">⚠ Lebih dari satu wajah terdeteksi. Gunakan foto satu orang.</p>
				{:else if faceValidation === 'blur'}
					<p class="text-sm text-error-400">⚠ Foto terlalu blur. Gunakan foto yang lebih tajam.</p>
				{/if}
			</fieldset>
